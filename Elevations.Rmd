...
This is multiple files. It'll download once, put into the folder in `path` and then load from there next time you run the code.
...
I had to add `outer.margins` so that it didn't cut off the y-axis tick labels.

```{r fig.width=10, dpi=300}
tiff(filename="BB_sockeye_rivers.tiff")
finalp + tm_layout(outer.margins=c(.02,.05,.01,.03))
dev.off()
```

png for rmarkdown docs.

```{r fig.width=10, dpi=300}
png(filename="BB_sockeye_rivers.png")
finalp + tm_layout(outer.margins=c(.02,.05,.01,.03))
dev.off()
```

```{r, echo=FALSE, fig.cap="A nice image."}
knitr::include_graphics("BB_sockeye_rivers.png")
```
